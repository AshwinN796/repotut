...
internal class NetworkResponseCall<S : Any, E : Any>(
    ...
    override fun enqueue(callback: Callback<NetworkResponse<S>>) = synchronized(this) {
        backingCall.enqueue(object : Callback<S> {
            override fun onResponse(call: Call<S>, response: Response<S>) {
                val networkResponse = ResponseHandler.handle(response, successBodyType)
                callback.onResponse(this@NetworkResponseCall, Response.success(networkResponse))
            }

            override fun onFailure(call: Call<S>, throwable: Throwable) {
                val networkResponse = throwable.extractNetworkResponse<S>()
                callback.onResponse(this@NetworkResponseCall, Response.success(networkResponse))
            }
        })
    }
    ...
    override fun clone(): Call<NetworkResponse<S>> = NetworkResponseCall(
        backingCall.clone(),
        errorConverter,
        successBodyType
    )
